...
def test_task_comments_with_no_comments_should_return_empty_list():
    task = shibboleth.Task.create_from_content(
        content=dedent(
            """
        Title: what the what?
...
@pytest.mark.parametrize("order", [1, 2, 5, "three-sir"])
...
def test_if_autocommit_and_task_update_then_updates_should_be_committed(
    shibby_with_git, tmp_path
):
    shibby = shibboleth.Shibboleth(root_dir=tmp_path)
    shibby.config["autocommit"] = True
